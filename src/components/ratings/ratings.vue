	...
	<section class="ratings" ref="scrollWrapper">
		<figure class="ratings-wrapper">
			...
			<div class="ratings__content">
				<ul>
					<li v-for="rating in ratings" class="content__unit" v-show="showThisRating(rating)">
						...
						<div class="unit__user">
							...
							<div class="user__stars-wrapper">
								...
								<span class="stars__delivery" v-show="rating.deliveryTime">{{rating.deliveryTime}}分钟送达</span>
							</div>
							<p class="user__text">{{rating.text}}</p>
							<div class="user__recommend" v-show="rating.recommend && rating.recommend.length">
								<span class="icon-thumb_up"></span>
								<span v-for="text in rating.recommend" class="recommend__item">{{text}}</span>
							</div>
		...
		.ratings__overview
			...
			.overview__right
				...
				.right__serviceScore-wrapper,.right__foodScore-wrapper
					...
					// line-height : 18px
					font-size : 0
					.serviceScore__title,.serviceScore__star,.serviceScore__point,
					.foodScore__title,.foodScore__star,.foodScore__point

  ...
  <section class="goods">
    ...
    <aside class="menu-wrapper" ref="menuWrapper">
      ...
      <ul>
        ...
        <li
          v-for="(goodsClassification, index) in goods"
          class="menu__item"
        ...
        >
          <span class="item__text border-1px">
            ...
            <span
              class="item__icon"
              :class="classMap[goodsClassification.type]"
              v-if="goodsClassification.type>0"
            ></span>
            {{goodsClassification.name}}
          </span>
        </li>
      </ul>
      <!-- 渲染导航列表 -->
    ...
    <figure class="goods-wrapper" ref="foodsWrapper">
      ...
      <ul>
        ...
        <li
          v-for="goodsClassification in goods"
          :key="goodsClassification.name"
          class="goods-item goods-item-hook"
        >
          <!-- 类别名称 -->
          <h1 class="goods__title">{{goodsClassification.name}}</h1>
          <!-- 类别名称 -->
          <ul>
            <!-- 遍历每个类别中的所有商品，每个商品作为第二层列表 -->
            <li
              v-for="foodItem in goodsClassification.foods"
              class="food-item border-1px"
              :key="foodItem.name"
              @click="showInfo(foodItem,$event)"
            >
              <!-- 商品图标 -->
              ...
              <div class="food__content">
                <!-- 商品名称 -->
                <h2 class="content__name">{{foodItem.name}}</h2>
                <!-- 商品名称 -->
                <!-- 商品描述 -->
                <p class="content__desc">{{foodItem.description}}</p>
                <!-- 商品描述 -->
                <!-- 月销量以及好评数 -->
                <div class="content__extral">
                  <span class="extral__count">月售{{foodItem.sellCount}}份</span>
                  <span>好评率{{foodItem.rating}}%</span>
                </div>
                <!-- 月销量以及好评数 -->
                <!-- 价格 -->
                <div class="content__price">
                  <span class="price__current">￥{{foodItem.price}}</span>
                  <span class="price__old" v-if="foodItem.oldPrice">￥{{foodItem.oldPrice}}</span>
                </div>
                <!-- 价格 -->
